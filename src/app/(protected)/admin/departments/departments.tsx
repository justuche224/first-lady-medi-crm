...
export default function DepartmentsPage() {
  ...
  const loadDepartments = useCallback(
    async (page = currentPage, search = searchTerm) => {
      setIsLoading(true);
      try {
        const result = await getDepartments(page, pageSize, search);
        if (result.success) {
  ...
  const getStats = () => {
    const departmentsWithHead = departments.filter(
      (d) => d.headDoctorId
    ).length;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage hospital departments and their leadership
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Departments without head doctor
            </p>
          </CardContent>
        </Card>
      </div>
