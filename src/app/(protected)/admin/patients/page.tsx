...
export default function PatientsPage() {
  ...
  const loadPatients = useCallback(
    async (page = currentPage, search = searchTerm) => {
      setIsLoading(true);
      try {
        const result = await getPatients(page, pageSize, search);
        if (result.success) {
  ...
  const getStats = () => {
    const activePatients = patients.filter((p) => !p.banned).length;
    const bannedPatients = patients.filter((p) => p.banned).length;
    const avgHealthScore =
      patients.reduce((sum, p) => sum + (p.healthScore || 0), 0) /
        patients.length || 0;

    return {
      total: pagination.total,
      active: activePatients,
      banned: bannedPatients,
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <>
              <PatientsTable patients={patients} onRefresh={handleRefresh} />
